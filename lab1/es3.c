...
corsa_t* cercaCodiceDicotomica(corsa_t* corse, int n, char* codice) { // cerca il codice con una ricerca binaria su un array ordinato di elementi.
	...
	orderByString(corse, n, 2); // assicurarsi che l'array sia ordinato
...
int main(int argc, char** argp){
	...
	while (comando != r_fine) {
		...
		switch(comando) {
			...
			case r_order:
				int option;
				printf("-\tPer cosa vuoi ordinare?\n- 1)\tData\n- 2)\tCodice Tratta\n- 3)\tStazione di Partenza\n- 4)\tStazione di Arrivo\n> ");
				scanf("%d", &option);
				switch(option) {
					case 1:
						orderByDate(corse, n);
			...
			case r_search:
				...
				switch (option) {
					case 1:
						corsa_t* result = lineare ? cercaCodiceLineare(corse, n, input) : cercaCodiceDicotomica(corse, n, input);
						if (result != NULL)
							printInfo(*result);
						else
					...
					case 2:
						if (lineare) {
							...
							int result_num;
							result = cercaStazioneDicotomica(corse, n, input, &result_num);
