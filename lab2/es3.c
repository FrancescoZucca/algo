...
int main(int argc, char** argp){
	...
	char filename[101];
	strncpy(filename, argp[1], 100);

	corsa_t* corse;
	int n = parseFile(filename, &corse);
	if (n == -1) {
	...
	while (comando != r_fine) {
		...
		switch(comando) {
			case r_load:
				...
				corse_date = malloc(n * sizeof(corsa_t *));
				corse_codice = malloc(n * sizeof(corsa_t *));
				corse_partenza = malloc(n * sizeof(corsa_t *));
				corse_arrivo = malloc(n * sizeof(corsa_t *));
